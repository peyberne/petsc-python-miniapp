    ...
    try:
        for repetition in range(repetitions):
            ...
            if regime == "setup":
                ...
                # The PC reuse flag must be OFF so PCSetUp proceeds. Re-assemble
                # the matrix (like SOLEDGE3x rebuilds it every time step): this
                # increases the matrix object state so PCSetUp sees the matrix
                # as modified, uses SAME_NONZERO_PATTERN (the sparsity pattern
                # is unchanged), and GAMG reuses the aggregates and the
                # prolongator while only recomputing the Galerkin products,
                # like SOLEDGE3x reUsePCMaxIt = 0.
                pc.setReusePreconditioner(False)
                mat.assemble()
                ksp.setOperators(mat)
                PETSc.COMM_WORLD.barrier()
